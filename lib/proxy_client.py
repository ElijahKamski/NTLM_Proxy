...
from lib import logger, http_header, ntlm_auth, basic_auth
...
class proxy_HTTP_Client:
    ...
    def run(self):
        ...
        while not self.stop_request:
            ...
            if self.client_head_obj is None and self.rserver_data_sent:
                self.reset_rserver()
        ...
        if self.rserver_buffer and (not self.rserver_data_sent):
            ...
            try:
                ...
                if self.rserver_all_got:
                    ...
                    self.logger.log(
                        '*** Sent ALL the data from remote server to client. (Server buffer - %d bytes)\n' % len(
                            self.rserver_buffer))
            except:
            ...
            self.logger.log(
                "*** No server's data to send to the client. (server's buffer - %d bytes)\n" % len(self.rserver_buffer))
    ...
    def guess_client_data_length(self):
        """"""
        if not self.client_head_obj.has_param('Content-Length') and not self.client_head_obj.has_param(
                'Transfer-Encode'):
            self.client_all_got = 1
            self.client_data_sent = 1
            self.client_data_length = 0
            self.logger.log(
                "*** Client request header does not have 'Content-Length' or 'Transfer-Encoding' parameter and it "
                "must not have any body.\n")

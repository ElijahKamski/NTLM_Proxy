...
def arrange(conf):
    ...
    if conf['GENERAL']['PARENT_PROXY']:
        ...
        conf['GENERAL']['PARENT_PROXY_TIMEOUT'] = makeInt(conf['GENERAL']['PARENT_PROXY_TIMEOUT'],
    ...
    try:
        ...
        if conf['GENERAL']['MAX_CONNECTION_BACKLOG'] == 'SOMAXCONN':
            ...
            print(
                "ERROR: There is a problem with 'MAX_CONNECTION_BACKLOG' in the config (neither a number nor 'SOMAXCONN'?)")
            sys.exit(1)
    conf['GENERAL']['LISTEN_PORT'] = makeInt(conf['GENERAL']['LISTEN_PORT'], 'LISTEN_PORT')

    conf['GENERAL']['ALLOW_EXTERNAL_CLIENTS'] = makeInt(conf['GENERAL']['ALLOW_EXTERNAL_CLIENTS'],
    ...
    try:
        ...
        print(
            "ERROR: Unable to get the IP address of this machine.  This is not a fatal problem, but may cause "
            "problems for you using this proxy in some scenarios.")
        externalIP = []
    ...
    url_logger_lock = threading.Lock()

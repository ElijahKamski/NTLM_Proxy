...
class monitorThread:
    ...
    def run(self):
        while self.alive:
            ...
            thread = threading.Thread(target=self.alarmThread.run, args=())
            thread.start()
    ...
    def httpTest(self):
        ...
        """
        if hasattr(http.client, 'HTTPConnection'):
            # Python 2.x
            try:
                conn = http.client.HTTPConnection(self.config['GENERAL']['PARENT_PROXY'],
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.die()
                        conn.close()
                    except (AttributeError, http.client.BadStatusLine):  # Didn't somehow connect?
                        self.die()
            ...
            try:
                ...
                # using Python 2.x
                conn = http.client.HTTPConnection(self.config['GENERAL']['PARENT_PROXY'], self.config['GENERAL']['PARENT_PROXY_PORT'])
                try:
                    conn.putrequest('GET', '/')
                    conn.endheaders()
                    try:
